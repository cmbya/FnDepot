...
def main():
    ...
    for item in cfg["repositories"]:
        ...
        if not appname or not version:
            raise RuntimeError(
                f"{repo}: FPK manifest 缺少 appname/version"
            )
        ...
        print(
            f"收录: {appname} {version} "
            f"({release_type}, run_as={run_as})"
        )
